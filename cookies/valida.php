<?php
			...
			function validaFormRegisto( $dados ){
				...
				$errors = array('username' => array(false, "Invalid username: it must have between $minUsername and $maxUsername chars."),
								    ...
								    'rpassword' => array(false, "Passwords mismatch."),
								    'email' => array(false,'Invalid email.'),
								    'gender' => array(false,'Please select a gender.')
								   );
				...
				if ( !is_array($dados) || count (array_intersect_key(array_keys($errors), array_keys($dados) ) ) < 4 ){
					return("This function needs a parameter with the following format: array(\"username\"=> \"value\", \"password\"=> \"value\", \"rpassword\"=> \"value\", \"email\"=> \"value\", [optional] \"gender\"=> \"value\")");
					die();
				}	
